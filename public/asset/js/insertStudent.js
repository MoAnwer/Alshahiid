		...
		.then((response) => {
			if (response.statusCode == 200) {
				alertify.showAccountData(
					`
						<p class="my-3 h6">اسم المستخدم للحساب : <b>${response.studentAccount.username}</b></p>
						<p class="mb-3 h6">كلمة المرور الحساب : <b>${response.studentAccount.password}</b></p>
				`
				);
...
document.querySelector("#alert").onclick = (e) => {
	...
	// if (!validate()) {
	// 	alertify.error(
	// 		' <i class="bi bi-x-circle-fill mx-1"></i>   قم بملء كل الحقول  !!'
	// 	);
	// 	return;
	// }
